#' Title
#' @title Project population scenarios across countries.
#' 
...
#' @param rlc River lengths for each scenario. From "res.Tab.R".
#'
...
#' 
#' }
demogCountry <- function(popc = NA, rlc = NA) {
  ...
  prepop <- function(x){
    ...
    dft <- reshape2::melt(riverl, id.vars = c('name', 'accessible'), 
    ...
    dft2 <- plyr::ddply(dft, c("name", "accessible", "variable"), myq)
  ...
  proj.rivl <- function(x){
    ...
    dout <- plyr::ddply(dfin, c("name", "accessible",
