...
#' @param demogCB Country and basin level population projections from "demogCountryBasin.R"
...
#' demogCB <- demogCountryBasin(popc = popc, rlcb = lt$rlc)
#' dfsum <- resTabDemogBasin(demogCB = demogC, make_html = FALSE)
#' }
resTabDemogBasin <- function(demogCB = NA, make_html = FALSE){
  ...
  dft.BAU <-merge(dfpop.res[selBA, ], dfpop.res[selBNA, ], 
                  by = c("namekey"))
  # summarise totals
    dfBAU <-  plyr::ddply(dft.BAU, c("namekey"), summarize,
  ...
  dfSP <-  plyr::ddply(dft.SP, c("namekey"), summarize,
  ...
  dfCM <-  plyr::ddply(dft.CM, c("namekey"), summarize,
  ...
  dfsum <- merge(dfBAU, dfSP, by = c("namekey"))
  dfsum <- merge(dfsum, dfCM, by = c("namekey"))  
  ...
  dfsum$CM_flag_30 <- ifelse(dfsum$CM_female_change < -29.999, 1,0) 
    ...
    if(make_html!=FALSE){
      tab1demog <- data.frame(basincountry = dfsum$namekey, 
    ...
    sink("inst/ms_res/tab1demogBasin.html")
